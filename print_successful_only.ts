...
function check(cipher: string, key: string, label: string) {
  try {
    const bytes = CryptoJS.AES.decrypt(cipher, key);
    const text = bytes.toString(CryptoJS.enc.Utf8);
    if (text.trim().length > 0) {
      console.log(`[SUCCESS msg] ${label} decrypted: "${text}"`);
    }
  } catch (e) {
    // ignore
  }
...
for (const dev of derivations) {
  check(msg1_cipher, dev.key, dev.name);
  check(msg2_cipher, dev.key, dev.name);
  for (const cid of chatIds) {
    if (cid) {
      const derivedKey = CryptoJS.SHA256(dev.key + cid).toString();
      check(msg1_cipher, derivedKey, `${dev.name} + chatId ${cid}`);
      check(msg2_cipher, derivedKey, `${dev.name} + chatId ${cid}`);
    }
  }
}
