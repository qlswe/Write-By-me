    ...
    <AnimatePresence>
      {isOpen && (
        <div 
          ...
          className="fixed inset-0 z-[1000] flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-md overflow-hidden"
        >
          <motion.div
            ...
            className="w-full max-w-md sm:max-w-lg bg-[#15101e] border border-[#3d2b4f]/70 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[86vh]"
          >
            {/* Header */}
            <div className="shrink-0 p-4 sm:p-5 border-b border-[#251c35] flex items-center justify-between bg-[#1b1427]">
            ...
            {/* Body */}
            <div className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-5 space-y-4 custom-scrollbar">
              {/* Story Live Preview Box */}
              <div className="flex justify-center">
                <div 
                  className={`w-44 h-60 sm:w-52 sm:h-72 rounded-2xl bg-gradient-to-b ${selectedGradient} p-3.5 sm:p-4 flex flex-col justify-between shadow-2xl relative overflow-hidden border-2 border-white/15`}
                >
                  ...
                  <div className="flex items-center gap-2 relative z-10">
                    <img
                      ...
                      alt="Author"
                      className="w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 border-white/80 object-cover shadow"
                    />
                  ...
                  <div className="relative z-10 my-auto text-center px-1.5">
                    <p className="text-xs sm:text-sm font-black text-white drop-shadow-md leading-relaxed break-words line-clamp-5">
                      {text || (lang === 'ru' ? 'Ваш текст истории появится здесь...' : 'Your story text will appear here...')}
                    </p>
                  </div>

                  <div className="relative z-10 flex justify-between items-center text-[9px] sm:text-[10px] text-white/70 font-mono">
                    <span>24h Story</span>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-fuchsia-400 hover:text-fuchsia-300 transition-colors disabled:opacity-50 cursor-pointer"
                  >
                ...
                <textarea
                  ...
                  maxLength={240}
                  className="w-full bg-[#0d0917] border border-[#3d2b4f]/60 rounded-2xl p-3 text-sm text-white placeholder-white/40 focus:outline-none focus:border-[#ff4d4d] min-h-[75px] resize-none"
                />
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {STORY_GRADIENTS.map((grad) => (
                      <button
                        ...
                        className={`h-9 rounded-xl bg-gradient-to-r ${grad.class} border-2 transition-all flex items-center justify-center cursor-pointer ${
            ...
            {/* Footer Actions */}
            <div className="shrink-0 p-4 sm:p-5 border-t border-[#251c35] flex items-center justify-end gap-3 bg-[#171022]">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-white/60 hover:text-white hover:bg-white/5 transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider bg-gradient-to-r from-[#ff4d4d] to-fuchsia-600 text-white hover:from-[#ff6666] hover:to-fuchsia-500 transition-all shadow-lg hover:shadow-[0_0_20px_rgba(255,77,77,0.3)] disabled:opacity-40 active:scale-95 cursor-pointer"
              >
                {isPublishing ? (
                  <>
