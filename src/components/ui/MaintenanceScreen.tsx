...
import { ShieldAlert, LogOut, Mail, MessageSquare, Lock } from 'lucide-react';
...
export const MaintenanceScreen: React.FC<MaintenanceScreenProps> = ({ 
  ...
  const [liveReason, setLiveReason] = useState<string>(() => {
    if (initialReason && initialReason.trim()) return initialReason;
    return safeStorage.getItem('aha_cached_maintenance_reason') || '';
  });
  const [liveUpdatedAt, setLiveUpdatedAt] = useState<number | undefined>(() => {
    if (initialUpdatedAt) return initialUpdatedAt;
    const cachedTime = safeStorage.getItem('aha_cached_maintenance_updated_at');
    return cachedTime ? parseInt(cachedTime, 10) : undefined;
  });

  // Real-time listener for maintenance settings directly in MaintenanceScreen
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'general'), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        const reason = data.maintenanceReason || '';
        const updated = data.maintenanceUpdatedAt || undefined;
        setLiveReason(reason);
        if (updated) setLiveUpdatedAt(updated);

        if (reason) {
          safeStorage.setItem('aha_cached_maintenance_reason', reason);
        }
        if (updated) {
          safeStorage.setItem('aha_cached_maintenance_updated_at', updated.toString());
        }
      }
    }, (err) => {
      console.warn("MaintenanceScreen settings subscription error:", err);
    });

    return () => unsub();
  }, []);

  const displayReason = liveReason || initialReason || '';
  const displayUpdatedAt = liveUpdatedAt || initialUpdatedAt;
    ...
    <div className="min-h-screen bg-[#0d0b14] flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden select-none">
      ...
      >
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-mono font-bold uppercase tracking-widest mb-6">
          ...
          <span>{loc('AHA-PROTOCOL: ДОСТУП ОГРАНИЧЕН', 'AHA-PROTOCOL: SITE LOCKED', 'AHA-PROTOCOL: ДОСТУП АБМЕЖАВАНЫ', 'AHA-PROTOKOLL: ZUGRIFF GESPERRT', 'PROTOCOLE-AHA : ACCÈS RESTREINT', 'AHA 协议：访问受限')}</span>
        </div>
        ...
        <h1 className="text-2xl sm:text-4xl font-black text-white mb-3 uppercase tracking-wider font-sans leading-tight">
          {t.maintenanceSiteClosed || loc('Сайт временно закрыт', 'Site Temporarily Closed', 'Сайт часова зачынены', 'Website vorübergehend geschlossen', 'Site temporairement fermé', '网站 Gem 闭维护中')}
        </h1>
        ...
        {displayReason && displayReason.trim().length > 0 ? (
          <div className="my-6 text-left bg-[#1a1226] border-l-4 border-[#ff4d4d] border-y border-r border-[#3d2b4f]/80 rounded-2xl p-4 sm:p-5 shadow-lg relative overflow-hidden group">
            <div className="flex items-center gap-2 text-[#ff4d4d] text-xs font-bold uppercase tracking-wider mb-2 font-mono">
              ...
              <span>{loc('Причина закрытия от администрации:', 'Official Closure Reason:', 'Прычына зачынення ад адміністрацыі:', 'Оffizieller Grund der Schließung:', 'Raison officielle de fermeture :', '来自管理员的关闭原因：')}</span>
            </div>
            ...
            {displayUpdatedAt && (
              <div className="mt-3 pt-2 border-t border-[#3d2b4f]/40 text-[11px] text-gray-500 font-mono text-right">
                {loc('Обновлено: ', 'Updated: ', 'Абноўлена: ', 'Aktualisiert: ', 'Mis à jour : ', '更新于：')}{new Date(displayUpdatedAt).toLocaleString(lang === 'ru' ? 'ru-RU' : lang === 'by' ? 'be-BY' : lang === 'de' ? 'de-DE' : lang === 'fr' ? 'fr-FR' : lang === 'zh' ? 'zh-CN' : 'en-US')}
              </div>
            )}
          </div>
        ) : (
          <p className="text-gray-300 mb-8 leading-relaxed text-sm sm:text-base font-sans">
            {t.maintenanceDesc || loc('На сайте проводятся технические работы. Приносим извинения за неудобства.', 'The site is currently undergoing scheduled maintenance.', 'На сайце праводзяцца тэхнічныя работы.', 'Auf der Website finden derzeit Wartungsarbeiten statt.', 'Le site est actuellement en maintenance.', '网站目前正在进行维护。')}
          </p>
        )}
        ...
        {user ? (
          <div className="space-y-4 pt-2">
            <div className="p-3.5 bg-[#0d0b14] border border-[#3d2b4f] rounded-2xl text-xs text-gray-400">
              {loc('Вы вошли как:', 'Logged in as:', 'Вы ўвайшлі як:', 'Angemeldet als:', 'Connecté en tant que :', '当前登录身份：')}{' '}
              <span className="text-white font-bold">{user.displayName || user.email}</span>
              <div className="text-red-400 text-[11px] mt-1 font-mono">
                {loc('У вашего аккаунта нет прав администратора для обхода блокировки.', 'Account does not have admin permissions to bypass lock.', 'У вашага акаўнта няма правоў адміністратара.', 'Konto verfügt nicht über Administratorrechte.', 'Le compte ne dispose pas de privilèges administrateur.', '您的账号没有管理员权限以绕过锁定。')}
              </div>
            </div>
            <button
              onClick={logout}
            ...
            >
              <LogOut size={16} />
              {t.maintenanceLogout || loc('Выйти из аккаунта', 'Log Out', 'Выйсці з акаўнта', 'Abmelden', 'Se déconnecter', '退出账号')}
            </button>
          </div>
        ) : (
          <div className="flex flex-col gap-3 w-full max-w-sm mx-auto pt-2">
            <p className="text-xs text-gray-400 mb-1">
              {loc('Вход для администраторов и персонала:', 'Staff & Administrator Access:', 'Уваход для адміністратараў і персаналу:', 'Zugang für Administratoren und Personal:', 'Accès administrateurs et personnel :', '管理员与工作人员登录：')}
            </p>
            ...
            >
              <Mail size={14} />
              {loc('Войти через почту / Email', 'Login via Email', 'Увайсці праз пошту / Email', 'Mit E-Mail anmelden', 'Connexion par e-mail', '通过电子邮箱登录')}
            </button>
          </div>
        )}
      </motion.div>
