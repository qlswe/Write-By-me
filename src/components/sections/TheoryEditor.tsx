...
export const TheoryEditor: React.FC<TheoryEditorProps> = ({ theory, onClose }) => {
  ...
  const [title, setTitle] = useState<Record<string, string>>(
    typeof theory?.title === 'object' ? theory.title : LANGUAGES.reduce((acc, lang) => ({ ...acc, [lang]: theory?.title || '' }), {})
  );
  const [summary, setSummary] = useState<Record<string, string>>(
    typeof theory?.summary === 'object' ? theory.summary : LANGUAGES.reduce((acc, lang) => ({ ...acc, [lang]: theory?.summary || '' }), {})
  );
  const [content, setContent] = useState<Record<string, string>>(
    typeof theory?.content === 'object' ? theory.content : LANGUAGES.reduce((acc, lang) => ({ ...acc, [lang]: theory?.content || '' }), {})
  );
